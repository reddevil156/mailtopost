...
class mime_parser
{
	/** @var \david63\mailtopost\pop3mail\rfc822_addresses */
	protected $rfc822_addresses;

	/**
	* Constructor for admin manage controller
	*
	* @param \david63\mailtopost\pop3mail\rfc822_addresses	$rfc822_addresses Mail rfc822 addresses class
	*
	* @access public
	*/
	public function __construct(rfc822_addresses $rfc822_addresses)
	{
		$this->rfc822_addresses = $rfc822_addresses;
	}
	...
	function ParseStructuredHeader($value, &$type, &$parameters, &$character_sets, &$languages)
	{
		...
		while (strlen($p))
		{
			...
			if (strlen($remaining) && !strcmp($remaining[0], '"') && (gettype($quote = strpos($remaining, '"', 1)) == 'integer'))
			{
			...
			case MIME_PARSER_BODY:
				if ($this->mbox)
				{
					...
					if ($this->FindLineBreak($this->buffer_position, $break, $line_break))
					{
						...
						if ($following >= strlen($this->buffer) || gettype($line=strpos($this->buffer, $break, $following))!='integer')
						{
							if (!$end)
							{
								$need_more_data = 1;
				...
				else
				{
					if (strlen($this->buffer)-$this->buffer_position)
					{
						...
						if (!$end_line)
						{
							if (gettype($line_break=strrpos($data, "\n"))=='integer' || gettype($line_break=strrpos($data, "\r"))=='integer')
							{
								$line_break++;
...
// ***************************
					...
					else if (isset($this->headers['QuotedPrintable']))
					{
					...
					else if (isset($this->headers['Base64']))
					{
					...
					switch ($type)
					{
						case 'HeaderValue':
							...
							if (!isset($decoded['Headers'][$header]))
							{
								...
								if ($this->extract_addresses && isset($this->address_headers[$header]))
								{
									$decoded['HeaderPositions'][$header] = $part['Position'];
								}
							}
							else if (gettype($decoded['Headers'][$header])=='string')
							{
								$h = 1;
						...
						if (($l = strlen($body)))
						{
							...
							for (; $position<$l;)
							{
								...
								if (count($headers))
								{
									...
									else if (isset($headers['final-recipient']))
									{
