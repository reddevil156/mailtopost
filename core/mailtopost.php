...
class mailtopost
{
	...
	* @access public
	*/
	public function __construct(config $config, auth $auth, request $request, user $user, language $language, log $log, driver_interface $db, functions $functions, $tables, pop3 $pop3, mime_parser $mime_parser, $phpbb_root_path, $php_ext, $mailtopost_constants)
	{
		$this->config				= $config;
		$this->auth					= $auth;
